...
for s = 1:length(s_space)
    for b = 1:length(b_space)
        disp([s,b])
        [a1, n1] = iterating(deg2rad(b_space(b)),deg2rad(s_space(s)),tyre,Pressure,Density,wr,Area,C_d,lrack,larm,lrod,rpinion,d,v,kfw,krw,Ws,wuf,wur,rcf,rcr,l,tf,tr,wfr,cog,kpi,caster,toe_f,toe_r);
        n_space(s,b) = n1;
        a_space(s,b) = a1; 
    end
...
function [acc,n] = iterating(beta,theta,tyre,Pressure,Density,wr,Area,C_d,lrack,larm,lrod,rpinion,d,v,kfw,krw,Ws,wuf,wur,rcf,rcr,l,tf,tr,wfr,cog,kpi,caster,toe_f,toe_r)
    ...
    while abs(lateral)>50 && count < 50
        ...
        while abs(thrust)>10 && it_count < 20
            ...
            %sr_r = (abs(vel_l)*sr+vel_l-vel_r)/abs(vel_r);
